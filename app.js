...
function doPost(e) {
  try {
    ...
    // Hero Action Card Listeners
    if (btnHeroLocalChurch) {
        btnHeroLocalChurch.addEventListener('click', () => {
            const dirSec = document.getElementById('directory-section');
            if (dirSec) dirSec.scrollIntoView({ behavior: 'smooth', block: 'start' });
        });
    }

    if (btnHeroLcc) {
        btnHeroLcc.addEventListener('click', () => {
            const dirSec = document.getElementById('directory-section');
            if (dirSec) dirSec.scrollIntoView({ behavior: 'smooth', block: 'start' });
        });
    }
    ...
    if (btnHeroDcc) {
        btnHeroDcc.addEventListener('click', () => {
            // Allow access to DCC Questionnaire for now as requested
            const dccChurchObj = {
                id: "DCC-EXEC",
                displayName: "ECWA Umuahia DCC Strategic Questionnaire",
                rawName: "ECWA Umuahia District Church Council",
                searchSlug: "dcc"
            };
            showSurveyPage(dccChurchObj, true);
        });
    }
